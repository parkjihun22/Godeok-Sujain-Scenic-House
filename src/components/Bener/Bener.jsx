...
const contents = (text, isLoaded) => {
  if (text === '홍보영상' || text === '체크포인트' || text === '당첨자서류안내' || text === '평택 고덕 제일풍경채') {
    return (
      <>
        ...
        <div className={`${styles.text} ${isLoaded ? styles.showText : ''}`}>
          평택 고덕 제일풍경채, 우수한 입지와 생활 인프라를 누리는 브랜드 아파트.
        </div>
        <div className={`${styles.text} ${isLoaded ? styles.showText : ''}`}>
    ...
    return (
      <>
        <div className={`${styles.text} ${isLoaded ? styles.showText : ''}`}>
          평택 고덕국제신도시 중심 입지에 조성되는 평택 고덕 제일풍경채.
        </div>
        <div className={`${styles.text} ${isLoaded ? styles.showText : ''}`}>
    ...
    return (
      <>
        <div className={`${styles.text} ${isLoaded ? styles.showText : ''}`}>
          쾌적한 주거 환경과 효율적인 공간 설계를 고려한 평택 고덕 제일풍경채.
        </div>
        <div className={`${styles.text} ${isLoaded ? styles.showText : ''}`}>
          다양한 커뮤니티와 조경 특화 설계로 일상 속 여유를 제공합니다.
        </div>
        <div className={`${styles.text} ${isLoaded ? styles.showText : ''}`}>
          평택 고덕 제일풍경채, 새로운 주거 기준을 제안합니다.
        </div>
      </>
    );
  }
